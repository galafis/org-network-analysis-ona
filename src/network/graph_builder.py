...
class OrgGraphBuilder:
    ...
    def build_from_interactions(
        ...
        # Aggregate weights
        agg = (
            df.groupby(["source_id", "target_id"])["weight"]
            .sum()
            .reset_index()
        )
        ...
        for emp_id, attrs in emp_attrs.items():
            self.graph.add_node(str(emp_id), **attrs)
    ...
    def build_directed(
        ...
        emp_attrs = employees_df.set_index("employee_id").to_dict("index")
        for emp_id, attrs in emp_attrs.items():
            self.directed_graph.add_node(str(emp_id), **attrs)
    ...
    def get_stats(self, graph: nx.Graph | None = None) -> GraphStats:
        ...
        avg_clust = (
            round(float(nx.average_clustering(g, weight="weight")), 4)
